...
<div class="mb-4">
    <h4 class="fw-bold mb-0">Buat Pesanan Laundry Baru</h4>
    ...
    <div class="col-12 col-md-8">
        <div class="card card-premium border-0">
            <div class="card-header-premium">
                <span>Form Input Pesanan</span>
            </div>
            <div class="card-body p-4">
                <!-- Display Validation Errors -->
                <?php $validation = session('validation'); ?>

                <form action="/pesanan/store" method="post" id="formPesanan">
                    <?= csrf_field() ?>

                    <!-- Pilih Pelanggan -->
                    <div class="mb-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <label for="pelanggan_id" class="form-label fw-semibold mb-0">Pilih Pelanggan <span class="text-danger">*</span></label>
                            <a href="/pelanggan/create" target="_blank" class="small text-decoration-none"><i class="fa-solid fa-user-plus me-1"></i> Pelanggan Baru?</a>
                        </div>
                        <select class="form-select <?= (isset($validation) && $validation->hasError('pelanggan_id')) ? 'is-invalid' : '' ?>" id="pelanggan_id" name="pelanggan_id">
                            ...
                            <?php foreach ($pelanggan as $p): ?>
                                <option value="<?= $p['id'] ?>" <?= old('pelanggan_id') == $p['id'] ? 'selected' : '' ?>><?= esc($p['nama']) ?> - <?= esc($p['telepon']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select <?= (isset($validation) && $validation->hasError('layanan_id')) ? 'is-invalid' : '' ?>" id="layanan_id" name="layanan_id">
                            ...
                            <?php foreach ($layanan as $l): ?>
                                <option value="<?= $l['id'] ?>" data-harga="<?= $l['harga_per_kg'] ?>" data-durasi="<?= $l['estimasi_jam'] ?>" <?= old('layanan_id') == $l['id'] ? 'selected' : '' ?>>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" class="form-control <?= (isset($validation) && $validation->hasError('berat_kg')) ? 'is-invalid' : '' ?>" id="berat_kg" name="berat_kg" value="<?= old('berat_kg') ?>" placeholder="0.00">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select <?= (isset($validation) && $validation->hasError('status_bayar')) ? 'is-invalid' : '' ?>" id="status_bayar" name="status_bayar">
                                ...
                                <option value="dp" <?= old('status_bayar') == 'dp' ? 'selected' : '' ?>>Uang Muka (DP)</option>
                    ...
                    <!-- Nominal Bayar -->
                    <div class="mb-3" id="nominalBayarContainer" style="display: none;">
                        <label for="nominal_bayar" class="form-label fw-semibold">Nominal Uang yang Dibayarkan (Rp)</label>
                        <div class="input-group">
                            <span class="input-group-text">Rp</span>
                            <input type="number" step="0.01" class="form-control <?= (isset($validation) && $validation->hasError('nominal_bayar')) ? 'is-invalid' : '' ?>" id="nominal_bayar" name="nominal_bayar" value="<?= old('nominal_bayar') ?>" placeholder="0">
                        </div>
                        <div class="text-danger small mt-1">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control" id="catatan" name="catatan" rows="3" placeholder="Contoh: Luntur, setrika gantung, lipatan rapi..."><?= old('catatan') ?></textarea>
                    </div>

                    <div class="d-flex justify-content-end gap-2">
                        <a href="/pesanan" class="btn btn-secondary-premium">Batal</a>
                        <button type="submit" class="btn btn-premium">Proses Pesanan</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="card card-premium border-0 bg-light-subtle h-100">
            ...
            <div class="card-body p-4 d-flex flex-column justify-content-between">
                ...
                <div class="alert alert-info border-0 rounded-4 small p-3 mb-0">
                    ...
                    Pastikan berat pakaian diisi dengan benar setelah ditimbang di kasir.
                </div>
            </div>
        </div>
    </div>
